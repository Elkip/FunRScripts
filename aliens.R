...
## Map by county
## Return a dictionary of lat/long coordinates and coresponding county
...
# pointsDF <- aliens_us[,c("longitude","latitude")]
...
aliens_w_county <- aliens_w_county |> 
    mutate(full_county = paste(tolower(state.name[match(toupper(aliens_w_county$state), state.abb)]),aliens_w_county$county, sep=","))
...
data("fips_codes")
fips <- fips_codes |>
    mutate(fips = as.numeric(paste(state_code, county_code, sep = "")),
           full_county = str_remove_all(tolower(paste(state_name, county, sep = ",")), 
                                        "\\b (?:county|parish)$")) |>
